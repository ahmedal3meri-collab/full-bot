...
async def list_notes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    header = f"📝 **ملاحظاتك ({len(notes)}):**\n\n" if lang == "ar" else f"📝 **Your Notes ({len(notes)}):**\n\n"
    lines = [f"• `{name}` — _{date[:10]}_" for name, date in notes]
    footer = "\n\nاستخدم `/get [الاسم]` لقراءة ملاحظة." if lang == "ar" else "\n\nUse `/get [name]` to read a note."
